...
export const cryptoHandler = async (event: any = {}): Promise<any> => {
  try {
    ...
    // Get category IDs
    const categoryIdMap = await getCategoryIds(
      categoryNames,
      coinMarketCapApiKey,
    );
    ...
    const categoryPromises = categoryNames.map(async (categoryName) => {
      ...
      const coins = await getTopCoinsByCategory(
        categoryId,
        50,
        coinMarketCapApiKey,
      );
    ...
    const historicalData = await getHistoricalDataForCoins(
      coinIds,
      coinMarketCapApiKey,
    );
...
function processCoins(
  ...
  for (const coinId in historicalData) {
    ...
    const pricesLast7Days = quotes
      .slice(-7)
      .map((quote: any) => quote.quote.USD.price);
    ...
    const pricePosition =
      ((currentPrice - priceLow) / (priceHigh - priceLow)) * 100;
    ...
    if (priceSwing >= 20 && pricePosition <= 30) {
      ...
      buyingDipsCoins.push({
        ...
        targetSellPrice: (currentPrice * 1.1).toFixed(4),
    ...
    for (let i = 1; i < quotes.length; i++) {
      ...
      const priceChange =
        ((currQuote.quote.USD.price - prevQuote.quote.USD.price) /
          prevQuote.quote.USD.price) *
        100;
    ...
    if (volumeSpikes) {
      ...
      const volumeChange7dPercent =
        ((volumeEnd - volumeStart7d) / volumeStart7d) * 100;
      ...
      if (volumeChange7dPercent > 5) {
        ...
        const totalScore =
          priceStabilityScore * 0.6 + volumeIncreaseScore * 0.4;
